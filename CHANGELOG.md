...
- **面积 / 柱状样式里线条与填充「贴不到一起」**。
  ...
  - 面积图的两个填充完全重叠，混出来的紫既不像蓝也不像红；
  - 柱状图的 `.position(by:)` 没生效，被默认**堆叠**成一根，高度变成两者之和
    （读数直接是错的）。

  现在统一改成**镜像排布**：下载画在零轴上方、上传镜像到轴下方 —— 吞吐图的标准
  画法，也和菜单栏「上行 / 下行」的隐喻一致。纵轴标签显示幅值，方向由上下位置与
  图例表达；零轴补一条淡线。柱状同时去掉 `position(by:)`（两侧本来就分开，
  再并排只会画歪）。

  顺带修掉三个渲染问题：

  - 面积样式里的 `LineMark` 漏改分组键（只按段分组），下载点与上传点被连成
    **竖线** —— 这正是填充「发紫」的直接原因：竖线从上到下穿过两个填充；
  - `.catmullRom` 平滑会在尖峰两侧**过冲到零轴另一侧**（速率不该出现「负」），
    改用 `.monotone`：一样平滑，但不过冲；
  - 面积镜像后必须保持 `.unstacked`：换成不带 `stacking` 参数的那几个重载会
    悄悄退回堆叠（0.7.x 修过一次的老问题，这次是用渲染位图量出来的）。

### Tests

195 个（+1）：把面积与柱状样式真的渲染成位图，断言下载整段在零轴上方、
上传整段在下方、两者不重叠；空洞断线的像素测试继续有效。

位图实测（24h 数据、对称突发）：蓝 141…327 / 红 328…512，正好在零轴两侧分开。

## [0.7.9] — 2026-09-18

### Fixed

- **时间线把两次突发之间的空洞连成了一条斜线**。

  07:58 和 14:08 各有一个点，中间六小时没有任何记录 —— 折线却把它们直连，
  看上去像这六个小时一直在传。现在按「有没有采集」区分两种缺口：

  - 采集器在跑、该进程没流量（同一分钟里别的进程有数据）→ 补齐为 **0**，
    画出来是贴地的线，表示「确实没传」；
  - 整分钟谁都没有数据（应用关了 / 机器睡了）→ **不返回该桶**，
    线段在那里断开，不再硬连。

  顺带修掉三个同源问题：

  - 画线改用 `series:` 按「**段 × 方向**」分组。只按段分组会把同一个点的
    下载值与上传值连起来，画成一条竖线（渲染测试里实测踩到）；
  - 孤点补一个小圆点并**显式上色** —— 自定义符号不继承 mark 的
    `foregroundStyle`，会落回系统强调色，上传方向的孤点曾变成蓝点；
  - x 轴固定成所选跨度：只有一两个点时不再自适应把轴压到那两个点上；
    光标落在空洞里也不弹 tooltip（那里没有数据，弹远处的读数只会误导）。

  真机数据（近 24h 的 iperf3）：250 个网格点 = 2 个有流量点 + 248 个补零，
  最大真空洞 70 分钟 —— 断在那里。

### Changed

- 查询一个从没传过流量的进程不再返回空数组，而是「有采集的桶 → 0」。
  语义从「没有数据」变成「数据是 0」，这也是折线能贴地的原因。

### Tests

194 个（+5）：有采集的空桶补 0 / 无采集的桶不返回、分段函数（相邻点跨度
超过一个桶即换段），以及把图**真的渲染成位图数像素** —— 空洞中点必须没有
线、两端孤点必须在（下载/上传都要，防止孤点配色回退成蓝）。
